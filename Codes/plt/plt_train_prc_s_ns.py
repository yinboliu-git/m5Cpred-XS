...
# 画roc svm - xgb - rf
def ks(y_predicted1, y_true1, y_predicted2, y_true2, y_predicted3, y_true3,name):
    ...
    plt.plot(fpr1, tpr1, 'b', label='XGBoost = %0.3f' % roc_auc1, color='Red')
    plt.plot(fpr2, tpr2, 'b', label='SVM = %0.3f' % roc_auc2, color='k')
...
def ks_2(y_predicted1, y_true1, y_predicted2, y_true2,name,model,mat):
    ...
    plt.plot(fpr1, tpr1, '--', label='AUROC(SHAP) = %0.3f' % roc_auc1, color='Red')
    plt.plot(fpr2, tpr2, ':', label='AUROC(No-SHAP) = %0.3f' % roc_auc2, color='Blue')
...
# prc
def ks_3(y_predicted1, y_true1, y_predicted2, y_true2,y_predicted3, y_true3,name,mat):
    import matplotlib.pyplot as plt
    import matplotlib
    matplotlib.use('Agg')
    Font = {'size': 16, 'family': 'Times New Roman'}
    ...
    average_precision1 = average_precision_score(y_true1, y_predicted1)
    average_precision2 = average_precision_score(y_true2, y_predicted2)
    average_precision3 = average_precision_score(y_true3, y_predicted3)
    plt.figure(figsize=(8,8),dpi=600)

    precision1, recall1, thresholds1 = precision_recall_curve(y_true1, y_predicted1)
    precision2, recall2, thresholds2 = precision_recall_curve(y_true2, y_predicted2)
    precision3, recall3, thresholds3 = precision_recall_curve(y_true3, y_predicted3)
    recall1[-1] = 0
    precision1[-1] = 0
    recall2[-1] = 0
    precision2[-1] = 0
    ...
    plt.plot(recall1, precision1, 'b', label='AUROC(XGBoost-SHAP) : %0.3f' % average_precision1, color='r')
    plt.plot(recall2, precision2, 'b', label='AUROC(SVM-SHAP) : %0.3f' % average_precision2, color='blank')
    plt.plot(recall3, precision3, 'b', label='AUROC(RF-SHAP) : %0.3f' % average_precision3, color='b')
