...
def get_all_method(pad_value,contrl_step = 'all',ps_ctrl = 0, best_feature_n = 0, root_file = '../', idx_sorted = 0) :
    # pad_value 就是 method
    ...
    if contrl_step != 'ps_n_other' and type(contrl_idx_sorted) == type(0):
            idx_sorted = get_shap_sort(shap_xlf_init, X_train, y_train)  # 获得shap排序（降序）

    if contrl_step == 'all' and  best_feature_n == 0:
    ...
    if type(contrl_idx_sorted) != type(0):
        return return_data_all, idx_sorted
    else:
